...
def main():
    ...
    local_spotify_song_list = localquery.local_query(graph, timeOfDay, speed_values[user_speed_choice]) # Returns 10 spotify songs from user data
    ...
    if(os.getenv('SKIP_SPOTIFY') == "0"):
        ...
        user_id = user['id']
        playlist_name = f"Hit-Me: A {speed_values[user_speed_choice]} for the {timeOfDay}"
